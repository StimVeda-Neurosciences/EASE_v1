    ...
    switch (event)
    {
        ...
        case ESP_GATTS_WRITE_EVT: /////////// gatt write event it is value recieve by the esp32
            if (!param->write.is_prep)
            {
                ...
                if (gatts_if == gatt_if_device_info)
                {
                    if (device_info_db_handle[DEVICE_NUMBER_VAL] == param->write.handle)
                    {
                        flash_app_write_info(flash_app_info_device_num, param->write.value, param->write.len);
                    }
                }

                if (gatts_if == gatt_if_custom)
                {
...
void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t* param)
{
    switch (event)
    {
        ...
        case ESP_GAP_BLE_SET_PKT_LENGTH_COMPLETE_EVT:
            ESP_LOGI(GATTS_TAG,
                     "packet length updated: rx = %d, tx = %d, status = %d",
...
esp_err_t esp_ble_send_err_indication(uint32_t err_code)
{
    if (conn_flag != connected)
        return ESP_ERR_INVALID_STATE;
    return esp_ble_gatts_send_indicate(gatt_if_custom,
                                       connection_id,
                                       custom_db_handle[ERROR_CODE_VAL],
                                       sizeof(err_code),
                                       u8_ptr(err_code),
                                       true);
}
